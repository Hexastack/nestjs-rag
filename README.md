...
| `apply-immediately` | Only allowed when impact is `none`/`query-only`. Creates a new immutable revision and copies its predecessor's existing documents/chunks/embeddings into a new physical snapshot — no re-chunking or embedding-provider calls, because nothing about the index inputs changed. Throws `RagReindexRequiredError` otherwise. |
| `stage` | Creates a `pending` revision. Does not touch the active revision. Re-index it explicitly later with `ragService.reindexRevision(...)` — on success the revision is marked `ready` and can then be switched in with `activateRevision(...)`; on failure it is marked `failed` and the active revision keeps serving. If active content changes during or after re-indexing, activation refuses the stale candidate; call `reindexRevision` again to rebuild it from a clean snapshot. If the active configuration revision changed, create a new staged update from that configuration instead. |
...
- **All source overrides are structurally validated before persistence.** This includes code-supplied defaults seeded during bootstrap as well as runtime updates; an invalid effective chunking/retrieval configuration fails without creating or changing a binding.
...
- **Reassigning a source removes its documents from the old profile.** `assignProfile` with `reindex-and-activate` switches the binding and removes the old serving copy only after a completely successful target sync; failed or partially successful attempts discard their target copy and leave the old binding/corpus intact. With `stage`, the old copy is removed immediately and the content is unavailable until you sync under the new profile. Archived revisions are untouched.
...
`updateProfile(name, patch, { expectedRevisionId })` implements compare-and-swap: if `expectedRevisionId` doesn't match the profile's current active revision, the call throws `RagConcurrencyError` instead of applying a stale change. Staging is guarded by the same active-revision comparison, so a pending revision cannot be persisted from a configuration that lost a race after the initial check.

The guarantee is enforced at write time, not just checked up front: every active-pointer flip (`applyImmediateRevision`, `activateRevision`) is a conditional update on both the active revision and a monotonic corpus generation. Every successful active ingest/update/delete advances that generation in its row transaction. A staged re-index records its source generation and can become ready/active only while both values still match. Therefore an ingest racing activation either commits first and makes the candidate stale, or sees the new active revision and rolls back—it can never report success and immediately disappear from search. A partial unique index (one active revision per profile) backs up the pointer CAS at the database level.
...
- **Every revision owns an immutable physical snapshot.** Re-indexed revisions build their own rows; `apply-immediately` revisions clone the existing rows under their own `dataRevisionId` without re-chunking/re-embedding. Subsequent writes and deletes affect only the active snapshot.
- **Rollback is exact.** `rollback(profileName, revisionId)` restores both that revision's configuration and the documents/chunks/embeddings it served when archived. Content that must disappear from all retained snapshots (for example, a compliance erasure) still requires explicit archived-revision cleanup.
- `activateRevision` validates chunk/embedding row-count consistency against the target's data revision (and, on Postgres, that the vector extension is actually installed) before flipping the active pointer, inside one transaction — old revision archived and new one activated atomically, or neither happens.
- `cleanupArchivedRevisions(profileName, { keep })` permanently deletes archived revisions older than the retention count, along with their snapshot rows and Postgres indexes. After cleanup, rollback to a purged revision fails with a descriptive `RagRevisionNotFoundError`.
