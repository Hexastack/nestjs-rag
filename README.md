...
| `apply-immediately` | Only allowed when impact is `none`/`query-only`. Creates a new (immutable) revision that shares its predecessor's existing chunks/documents/embeddings via `dataRevisionId` — no re-chunking, re-embedding, or row updates, because nothing about the index changed. Throws `RagReindexRequiredError` otherwise. |
| `stage` | Creates a `pending` revision. Does not touch the active revision. Re-index it explicitly later with `ragService.reindexRevision(...)` — on success the revision is marked `ready` and can then be switched in with `activateRevision(...)`; on failure it is marked `failed` and the active revision keeps serving. |
...
- **Index rows belong to the revision that built them.** Every revision carries a `dataRevisionId` — the id of the revision whose re-index produced the physical documents/chunks/embeddings it serves. A revision created by a re-index references itself; a revision created by an `apply-immediately` (query-only) update inherits its predecessor's `dataRevisionId`, so an entire query-only lineage shares **one live row set** and rows are never moved or copied.
- **Deletes are scoped to a data revision, like writes.** A record deletion seen during sync (or `removeSourceRecord`) removes the document from the active revision's data revision only; row sets built by other re-indexes keep their copy until `cleanupArchivedRevisions` purges them, so rollback across a re-index boundary restores exactly what that revision indexed. Within a query-only lineage the corpus is shared and *live*: rolling back a query-only change restores the previous configuration exactly, but serves the current shared corpus (including documents added or removed since), not a point-in-time copy. Content that must disappear from *all* retained row sets (e.g. a compliance erasure) requires explicit cleanup of archived revisions.
...
- `rollback(profileName, revisionId)` reactivates an older `ready`/`archived` revision without rebuilding or moving anything, as long as its index data hasn't been purged: activation is a pure pointer flip, and the restored revision serves whatever row set its `dataRevisionId` references — in either direction along the revision history.
- `cleanupArchivedRevisions(profileName, { keep })` (beyond the six-method core API) permanently deletes archived revisions older than the retention count, along with their documents/chunks/embeddings and Postgres indexes. An archived revision whose row set is still referenced by another retained revision (e.g. the indexing ancestor of the currently active query-only revision) is skipped, never purged out from under it. After cleanup, `rollback` to a purged revision fails with a descriptive `RagRevisionNotFoundError`.
