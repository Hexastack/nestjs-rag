...
1. **Every revision owns an immutable index snapshot.** Re-indexed revisions build rows under their own `dataRevisionId`; query-only (`apply-immediately`) revisions copy the current documents/chunks/embeddings/FTS rows under their own id without repeating chunking or embedding calls. Writes and deletes affect only the active snapshot, so rollback restores exactly the configuration and corpus that revision served. Every active mutation increments the profile's corpus generation in the same transaction; a staged re-index may activate only if its source revision and generation are still current.
...
8. **The SQLite FTS table must stay consistent with `rag_chunks`.** Every code path that inserts, copies, or deletes chunks must do the matching FTS write *inside the same transaction*. Postgres per-revision index DDL is best-effort and runs after the row transaction commits.
